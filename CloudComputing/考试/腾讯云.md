...
[云开发工程师上机指导](https://cloudpartner.lexiangla.com/docs/1e487eac09fa11eb87699ac1da38b4de?company_from=cloudpartner)
...
java -version
wget https://mirror-hk.koddos.net/apache/tomcat/tomcat-8/v8.5.70/bin/apache-tomcat-8.5.70.tar.gz
mkdir /opt/tomcat8.5        # 存放tomcat安装包
mkdir -p /opt/tbook/pics    # 存放应用图片
tar -zxvf apache-tomcat-8.5.70.tar.gz -C /opt/tomcat8.5
# 开启服务
cd /opt/tomcat8.5/apache-tomcat-8.5.70/bin/
./startup.sh
# 访问测试 http://119.29.117.25:8080/
# 设置开机自启动
echo "/opt/tomcat8.5/apache-tomcat-8.5.70/bin/startup.sh" >> /etc/rc.d/rc.local
```

注意：
```
CentOS 登录 ssh root@ip
Ubuntu 登录 ssh ubuntu@ip
```
...
use tbook;
source tbook.sql;
# 挂载云硬盘[数据盘]到mysql云服务器
sudo su
fdisk -l
mkfs.ext4 /dev/vdb  # 格式化磁盘
mkdir /data/mysql   # 创建新挂载点，用于把磁盘挂载到该目录
mount /dev/vdb /data/mysql  # 挂载磁盘到/data/mysql目录
df -TH                      # 查看挂载信息
# 设置开机自动挂载磁盘
ls -l /dev/disk/by-id       # 查看弹性云硬盘的软链接，用于自动挂载配置
echo "/dev/disk/by-id/virtio-disk-jh8gl7ii    /data/mysql ext4    defaults    0   2" >> /etc/fstab
mount -a                    # mount -a 的意思是将/etc/fstab的所有内容重新加载，运行通过说明写入成功
# 配置mysql数据保存到上一步挂载云硬盘中
service mysql stop
# 把MySQL现有数据迁移到/data/mysql
mv /var/lib/mysql/* /data/mysql     # 迁移数据到/data/mysql目录
chown mysql:mysql -R /data/mysql/   # 修改mysql数据库目录权限
# 修改mysql的配置文件中的datadir配置为/data/mysql，保存配置后重启
vim /etc/mysql/mysql.conf.d/mysqld.cnf
# 修改/etc/apparmor.d/usr.sbin.mysqld文件，修改文件中的Allow data dir access目录为/data/mysql
vim /etc/apparmor.d/usr.sbin.mysqld
service apparmor reload             # 重新加载apparmor配置
# 修改/usr/share/mysql/mysql-systemd-start，将原datadir配置的路径"/var/lib/mysql"修改为"/data/mysql"
vim /usr/share/mysql/mysql-systemd-start
service mysql restart
mysql -u root -p
show variables like '%datadir%';    # 显示修改后的data路径
```

- **弹性伸缩**

1、创建 [文件共享存储](https://console.cloud.tencent.com/cfs/overview)
2、挂载文件系统
```
# CentOS: sudo yum install nfs-utils
# Ubuntu: sudo apt-get install nfs-common
```
查看[挂载点信息](https://console.cloud.tencent.com/cfs/fs/detail?rid=1&id=cfs-pktq8plj&tab=mountInfo)，找到挂载点ip
```
mkdir -p /tbook/pics
sudo mount -t nfs -o vers=4 <挂载点ip>:/ <待挂载目标目录>
sudo mount -t nfs -o vers=4,noresvport 172.16.0.15:/ /tbook/pics
df -TH
# 设置开机自动挂载
echo "172.16.0.15:/ /tbook/pics nfs4 nfsvers=4,hard,timeo=600,retrans=2,_netdev 0 0" >> /etc/fstab
mount -a
```
3、修改应用图片上传路径
```
mv /opt/tbook/pics/* /tbook/pics/
```
4、创建集群自定义镜像
5、创建 [弹性伸缩](https://console.cloud.tencent.com/autoscaling) 的 [启动配置](https://console.cloud.tencent.com/autoscaling/config?rid=1)
```
这里根据提示给角色授权
选择创建的自定义镜像
```
6、创建伸缩组，配置 [负载均衡](https://console.cloud.tencent.com/clb)
```
创建负载均衡
配置监听器并在监听器下创建转发规则(域名暂时可胡填)
配置健康检查
配置安全组
将负载均衡配置在伸缩组
```
测试 [http://<LB IP>:8080/](http://1.14.168.202:8080/)
...
```
df -TH
# 模拟磁盘被占满，500M*50=25000M，需要等待1分钟
dd if=/dev/zero of=/data/mysql/tmpfile bs=500M count=50
df -TH
# 磁盘占满，重启mysql失败
service restart mysql
# 控制台扩容数据盘，几秒钟后扩容成功
# 扩展分区及文件系统，参考:https://cloud.tencent.com/document/product/362/53363
fdisk -l
service mysql stop  # 停止服务
umount /data/mysql  # 下载分区
wget -o /tmp/devresize.py https://raw.githubusercontent.com/tencentyun/tencentcloud-cbs-tools/master/devresize/devresize.py
python /tmp/devresize.py /dev/vdb   # 若报错，执行修复（fsck -a /dev/vdb）再次执行
mount /dev/vdb /data/mysql  # 挂载分区
df -TH  # 检查挂载信息
service mysql restart  # 重启服务
```

扩容脚本需要翻墙，不方便的同学直接复制以下内容
...
- 标准存储 (热点视频、社交图片、移动应用、游戏程序、动态网站)
- 低频存储 (网盘数据、大数据分析、政企业务数据、低频档案、监控数据)
- 归档存储 (档案数据、医疗影像、科学资料、影视素材)

1、创建存储桶 [https://console.cloud.tencent.com/cos5](https://console.cloud.tencent.com/cos5)
...
1、创建容器集群 [https://console.cloud.tencent.com/tsf](https://console.cloud.tencent.com/tsf)
